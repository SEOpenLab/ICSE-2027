...
def convert_examples_to_features(examples, seq_length, tokenizer):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        tokens.append(tokenizer.cls_token) #[CLS] #-id-0 -49154
        input_type_ids.append(0)
        for token in cand_tokens:
            tokens.append(token)
            input_type_ids.append(0)
        tokens.append(tokenizer.sep_token)  #[SEP] #-id-2 -49153
        input_type_ids.append(0)
        ...
        while len(input_ids) < seq_length:
            input_ids.append(tokenizer.pad_token_id) #[PAD] #-1 -49152
            input_mask.append(0)
            input_type_ids.append(0)
...
def prepare_tokenizer(tokenizer_class, tokenizer_name, do_lower_case=False):
    MASK_TOKEN = "<mask>"
    SEPARATOR_TOKEN = "<sep>"
    PAD_TOKEN = "<pad>"
    CLS_TOKEN = "<cls>"
...
if __name__ == '__main__':
    ...
    task_codes    = ['KTX', 'IDN', 'CPX', 'AST'] # TODO: put tasks in tasks.config ['KTX', 'IDN', 'LEN', 'TYP', 'REA', 'JBL', 'SRI', 'SRK', 'SCK', 'OCU', 'VCU', 'CSC', 'MXN', 'CPX', 'NPT']
    shuffle_kinds = ['ORIG']
    label_counts  = [ '1k', '10k'] #['100', '1k', '10k']
        ...
        for shuffle_kind in shuffle_kinds:
            for model_checkpoint in list(model_checkpoints.keys()):
                for label_count in label_counts:
                    ...
                    if model_checkpoint in ["CodeBERT-base", "CodeBERTa", "GraphCodeBERT-base", "UnixCoder-base", "UnixCoder-base-unimodal"]:
                    ...
                    elif model_checkpoint in ["StarEncoder"]:
                        model_path = sys.path[0] + "/tmp/" + modelname
                        config    = AutoConfig.from_pretrained(model_path, output_hidden_states=True)
                        tokenizer = prepare_tokenizer(AutoTokenizer, model_path)
                        model     = AutoModel.from_pretrained(model_path, config=config )
                        model.resize_token_embeddings(len(tokenizer)) #tokenizer 和 model 是配套的, this is  meaningless
