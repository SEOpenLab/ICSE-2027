...
def convert_examples_to_features(examples, seq_length, tokenizer):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        #print(cand_tokens)
        if len(cand_tokens) > seq_length - 1: 
            ## Account for [CLS] and [SEP] with "- 1"
            cand_tokens = cand_tokens[0:(seq_length - 1)] 
        ...
        for token in cand_tokens:
            tokens.append(token)
            input_type_ids.append(0)
        tokens.append(tokenizer.eos_token)  #[SEP] #-id-102
        input_type_ids.append(0)
        ...
        while len(input_ids) < seq_length:
            input_ids = [pad_id] + input_ids #[PAD] #-0
    ...
    with open(json_features, "w") as writer:
        with torch.no_grad():
            for input_ids, input_mask, example_indices in eval_dataloader: # batch_sized input_ids, input_mask, example_indices tensor
                ...
                all_outputs = model(input_ids=input_ids,  attention_mask=input_mask) 
...
if __name__ == '__main__':
    ...
    task_codes    = ['AST'] # 'KTX', 'IDN', 'CPX',  TODO: put tasks in tasks.config ['KTX', 'IDN', 'LEN', 'TYP', 'REA', 'JBL', 'SRI', 'SRK', 'SCK', 'OCU', 'VCU', 'CSC', 'MXN', 'CPX', 'NPT']
    shuffle_kinds = ['ORIG']
    label_counts  = ['1k'] #['100', '1k', '10k']

    model_checkpoints = { # TODO: put models in models.config OR make a models class

        "CodeGPT-small-java":          "microsoft/CodeGPT-small-java", 

        "CodeGen-small-multi":  "Salesforce/codegen-350M-multi",   
 
        "PolyCoder-small":  "NinedayWang/PolyCoder-160M",     

    }

    model_max_seq_lengths = {

        "CodeGPT-small-java":          1024,

        "CodeGen-small-multi":  2048,   

        "PolyCoder-small":  2048,     
        ...
        for shuffle_kind in shuffle_kinds:
            for model_checkpoint in list(model_checkpoints.keys()):
                for label_count in label_counts:
                    ...
                    if model_checkpoint in ["CodeGPT-small-py","CodeGPT-small-java","CodeGPT-small-py-adapt",
    "CodeGPT-small-java-adapt","CodeGen-small-multi","CodeGen-small-mono","PolyCoder-small","PolyCoder-base"]:
