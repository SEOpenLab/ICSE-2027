...
def classify_and_predict(train_X, train_y, dev_X, dev_y, test_X, test_y, feat_dim, num_classes):
    ...
    # testing
    test_acc = round(100*clf.score(test_X, test_y), 3)
    ...
    outpatx = sys.path[0] + '/outputs/' + task_code + '/' + model_kind + '+' + head + '_' + str(label_count) + '_' + shuffle_kind + '_' + str(eval_layer) + '_accuracy_' +'.csv'
...
if __name__ == "__main__":
    ...
    model_kinds   = {
        "CodeGPT-small-py":          13, 
        "CodeGPT-small-java":          13,
        "CodeGPT-small-py-adapt":          13, 
        "CodeGPT-small-java-adapt":          13, 
        "CodeGen-small-multi":  21,   
        "CodeGen-small-mono":  21,     
        "PolyCoder-small":  13,     
                    }

    label_counts  = ['1k'] #['100', '1k', '10k']
    task_codes    = ['IDN', 'CPX', 'AST'] #['KTX', 'IDN', 'LEN', 'TYP', 'REA', 'JBL', 'SRI', 'SRK', 'SCK', 'OCU', 'VCU', 'CSC', 'MXN', 'CPX', 'NPT'] 
